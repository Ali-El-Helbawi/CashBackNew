...
import {StatusBar, I18nManager, Dimensions} from 'react-native';
    ...
    <>
      ...
      {props.card && (
        <ButtonHeader
          ...
          <Fontisto name="history" size={16} color="#ffffff96" />
        </ButtonHeader>
      )}
    </>
  );
    ...
    <>
      {pageName == 'Home' ? (
        <ButtonHeader
          ...
          <SLIcon name="lock" size={18} color="#ffffff96" />
        </ButtonHeader>
      ) : (
        <ButtonHeader
...
function CustomHeader(props, {title = '', rightBtns, leftBtns}) {
  const {userInfo, userAccounts} = useContext(DataContext);
  ...
  return (
    <View renderToHardwareTextureAndroid={true}>
      ...
      <AnimatedLinearGradient
        ...
        <Column center={true}>
          <HeaderTitle isVIP={isVIP}>{props.title}</HeaderTitle>
        </Column>
        <Column>
          {/* <HeaderLeftBtns
            card={isCards}
