...
function ReceiveSMS(props) {
  ...
  function sendAmount() {
    ...
    let Timer = setTimeout(() => {
      ...
      console.log(value);
      console.log(
        FromAccount.substring(FromAccount.length, 6) + value.padEnd(6, '0'),
      );
      const enc2 =
        FromAccount.substring(FromAccount.length, 6) + value.padEnd(6, '0');
      const enc =
  ...
  return (
    <Container>
      <VerifyModal
        ...
        title={Translate('Receive Using SMS')}
        subtitle={Translate('Please Verify sender phone number and the amount')}
      ...
      <KeyboardAwareScrollView>
        <Body>
          <TitleText>{Translate('Receive Using SMS')}:</TitleText>
          <SubtitleText>
            {Translate('Enter sender phone number and the amount')}
          </SubtitleText>
          <TextField
            onChangeText={text => setFromAccount(text)}
            keyboardType="phone-pad"
            label={Translate('From Phone Number')}
            labelTextStyle={{fontFamily: Font}}
            titleTextStyle={{fontFamily: Font}}
            affixTextStyle={{fontFamily: Font}}
            error={PhoneError}
...
export default connect(mapStateToProps, mapDispatchToProps)(ReceiveSMS);
